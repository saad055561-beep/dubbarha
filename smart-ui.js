...
function runSmart(){
 try{
  ...
  const value=(rest.length?rest:top).slice().sort((a,b)=>(b.rating/Math.max(1,b.price))-(a.rating/Math.max(1,a.price)))[0];
  const cheap=(rest.length?rest:top).slice().sort((a,b)=>a.price-b.price)[0];
  const summary='<div class="why"><b>🏆 الأفضل لك:</b> '+best.name+'<br><b>💰 الأفضل قيمة:</b> '+value.name+'<br><b>🪙 الأرخص:</b> '+cheap.name+'</div>';
