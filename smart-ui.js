...
function runSmart(){
 try{
  ...
  const affiliate=(store,product)=>{const u=window.dabbarhaAffiliateUrl&&window.dabbarhaAffiliateUrl(store,product);if(!u)return '<span style="flex:1;text-align:center;background:#f3f3f3;color:#888;padding:11px;border-radius:11px;font-weight:bold">'+(store==='noon'?'🛍️ نون قريبًا':store==='temu'?'🛒 Temu قريبًا':'متجر قريبًا')+'</span>';return '<a href="'+u+'" target="_blank" rel="noopener" onclick="dabbarhaAffiliateClick(\''+store+'\','+JSON.stringify(product)+')">'+(store==='amazon'?'🛒 أمازون':'🛍️ '+((window.DABBIRHA_AFFILIATE[store]||{}).name||store))+'</a>'};
